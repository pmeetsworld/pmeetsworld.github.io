...
function DistroDriveSection({ items, vsGet, onTap, onMarkAll, tc }) {
    ...
    open && h('div', null,
      ...
      famOrder.map(fam =>
        h('div', { key: fam },
          ...
          grouped[fam].map((item, i) => h(DistroRow, { key: i, item, state: vsGet(item.sku), onTap: () => onTap(item.sku) }))
        )
      )
    )
  );
...
function DistroRow({ item, state, onTap }) {
  ...
  return h('div', { className: 'void-row', style: { opacity: state === 1 ? 0.45 : 1 } },
    h('div', { className: 'void-row__sku', style: { fontSize: 12, textDecoration: state === 1 ? 'line-through' : 'none' } },
      item.sku
    ),
    h('button', {
      className: 'btn--state',
