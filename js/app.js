...
function reducer(state, action) {
  switch (action.type) {
    ...
    // Weekly objective — resets each week, lives in weekly state
    case 'OBJ': {
      const obj = { ...(state.weekly.obj || {}), [action.key]: action.value };
      return { ...state, weekly: { ...state.weekly, obj } };
    }
    case 'DS': {
...
function VoidSection({ title, subtitle, items, states, onTap, accentColor, stateDef, isObj }) {
  ...
  return h('div', { className: 'card', style: { marginBottom: 8 } },
    ...
    open && h('div', { className: 'collapse-body' },
      items.length > 1 && h('div', { className: 'void-toggle-bar' },
        h('button', {
          className: 'btn btn--sm btn--pill',
...
function DistroDriveSection({ items, allPersistVs, vsGet, onTap, onMarkAll, tc }) {
  const [open, setOpen] = useState(false); // collapsed by default

  const total   = items.length;
  const soldIn  = items.filter(i => vsGet(i.sku) === 1).length;

  // Count how many accounts territory-wide have each SKU sold in
  function countTerritorySoldIn(sku) {
    let count = 0;
    Object.values(allPersistVs).forEach(acct => {
      if (acct.dd && acct.dd[sku] === 1) count++;
    });
    return count;
  }
  const openCnt = total - soldIn;
  const pct     = total > 0 ? Math.round((soldIn / total) * 100) : 0;

  // Primary grouping: Y (Needs Re-Buy) → B (Re-Buy) → R (NON-BUY / New)
  // Secondary grouping: brand family within each color section
  const COLOR_SECTIONS = [
    { type: 'Y', label: '🟡 Needs Re-Buy',     bg: '#FFFBEB', border: '#FDE68A', fg: '#B45309' },
    { type: 'B', label: '🔵 Re-Buy',            bg: '#EFF6FF', border: '#BFDBFE', fg: '#1D4ED8' },
    { type: 'R', label: '🔴 NON-BUY (new)',     bg: '#FFF5F5', border: '#FECACA', fg: '#991B1B',
      note: 'never sold or >6 months ago' },
  ];

  // Build grouped[section_type][family] = [items]
  const bySectionFamily = { Y: {}, B: {}, R: {} };
  items.forEach(item => {
    const t = item.type || 'Y';
    const fam = ddBrandFamily(item.sku);
    if (!bySectionFamily[t]) bySectionFamily[t] = {};
    if (!bySectionFamily[t][fam]) bySectionFamily[t][fam] = [];
    bySectionFamily[t][fam].push(item);
  });
  // Sort each family group: open first, sold-in last
  Object.values(bySectionFamily).forEach(famMap => {
    Object.values(famMap).forEach(group => {
      group.sort((a, b) => (vsGet(a.sku) === 1 ? 1 : 0) - (vsGet(b.sku) === 1 ? 1 : 0));
    });
  });
  ...
  return h('div', { className: 'card', style: { marginBottom: 8, border: '2px solid #B45309' } },
    ...
    h('button', { className: 'collapse-btn', style: { background: '#FFFBEB' }, onClick: () => setOpen(!open) },
      h('div', { className: 'row' },
        h('span', { className: 't-label', style: { color: '#B45309' } }, '⭐ Distro Drive'),
        h('span', { className: 't-muted', style: { fontSize: 12, marginLeft: 6 } }, soldIn + '/' + total + ' sold in')
      ),
      h('div', { className: 'row' },
        openCnt > 0 && h('span', {
        ...
        soldIn === total && h('span', {
          style: { background: '#166534', color: '#fff', borderRadius: 10, padding: '2px 8px', fontSize: 11, fontWeight: 700 }
        }, '✓ Complete'),
        h('span', { className: 'collapse-btn__chevron' }, open ? '▲' : '▼')
      )
    ),
    // Progress bar — always visible
    h('div', { style: { height: 4, background: '#FED7AA', margin: '0' } },
      h('div', { style: { height: 4, width: pct + '%', background: '#B45309', transition: 'width 0.3s', borderRadius: '0 2px 2px 0' } })
    ),
    // Body
    open && h('div', null,
      // Mark All button
      openCnt > 0 && h('div', { style: { padding: '8px 12px', borderBottom: '0.5px solid #FDE68A', background: '#FFFBEB', display: 'flex', justifyContent: 'flex-end' } },
        h('button', {
          className: 'btn btn--sm btn--pill',
          style: { fontSize: 12, border: '1.5px solid #B45309', color: '#B45309', background: '#fff' },
          onClick: (e) => { e.stopPropagation(); onMarkAll(); }
        }, 'Mark All Sold In')
      ),
      // Render Y → B → R sections, each with brand family sub-groups
      COLOR_SECTIONS.filter(sec => Object.keys(bySectionFamily[sec.type] || {}).length > 0).map((sec, si) => {
        const famMap  = bySectionFamily[sec.type];
        const famList = DD_BRAND_ORDER.filter(f => famMap[f]);
        return h('div', { key: sec.type },
          // Section header
          h('div', { style: { padding: '6px 12px 4px', background: sec.bg,
              borderTop: si > 0 ? '1.5px solid #E5E7EB' : 'none',
              borderBottom: '0.5px solid ' + sec.border } },
            h('span', { style: { fontSize: 11, fontWeight: 700, color: sec.fg } }, sec.label),
            sec.note && h('span', { style: { fontSize: 10, color: '#9CA3AF', marginLeft: 8 } }, sec.note)
          ),
          // Brand families within section
          famList.map(fam =>
            h('div', { key: fam },
              h('div', { style: { padding: '4px 12px 2px', background: '#FAFAFA',
                  borderBottom: '0.5px solid #F0F0F0' } },
                h('span', { style: { fontSize: 10, fontWeight: 700, color: '#9CA3AF',
                    textTransform: 'uppercase', letterSpacing: '0.4px' } }, fam)
              ),
              famMap[fam].map((item, i) => {
                const target = DD_TARGETS[item.sku] || 1;
                const soldIn = countTerritorySoldIn(item.sku);
                return h(DistroRow, {
                  key: i, item, state: vsGet(item.sku), onTap: () => onTap(item.sku),
                  territorySoldIn: soldIn, territoryTarget: target
                });
              })
            )
          )
...
function DistroRow({ item, state, onTap, territorySoldIn, territoryTarget }) {
  const st = DD_STATES[state] || DD_STATES[0];
  const targetMet = territorySoldIn !== undefined && territorySoldIn >= territoryTarget;
  const effective = state === 1 || targetMet;
  const typeLabel = item.type === 'R' ? { text: 'NEW', bg: '#FEE2E2', fg: '#991B1B' }
                 : item.type === 'B' ? { text: 'RE-BUY', bg: '#DBEAFE', fg: '#1D4ED8' }
                 : null; // Yellow = no badge (most common, no noise)
  return h('div', { className: 'void-row', style: { opacity: effective ? 0.45 : 1 } },
    h('div', { className: 'void-row__sku', style: { fontSize: 12, textDecoration: effective ? 'line-through' : 'none', flex: 1 } },
      item.sku,
      typeLabel && h('span', { style: { marginLeft: 6, fontSize: 9, fontWeight: 700, color: typeLabel.fg, background: typeLabel.bg, borderRadius: 6, padding: '1px 5px' } }, typeLabel.text),
      targetMet && state === 0 && h('span', { style: { marginLeft: 6, fontSize: 10, fontWeight: 700, color: '#166534', background: '#DCFCE7', borderRadius: 8, padding: '1px 6px' } }, 'Target Met')
    ),
    h('button', {
      className: 'btn--state',
...
// ── Account Card ──────────────────────────────────────────────────────────────
function AccountCard({ acct, data, weeklyDone, weeklyObj, persist, day, dispatch, onBack }) {
  const [helperOpen, setHelperOpen] = useState(false);
  const notesRef = useRef(null);
  const pageRef  = useRef(null);
  // Scroll to top each time a different account card opens
  useEffect(() => {
  ...
  const health = accountHealthScore(id, data, persist);
  ...
  return h('div', { className: 'page', ref: pageRef },
    // Sticky header
    ...
    // Body
    h('div', { className: 'pad stack', style: { paddingTop: 12 } },      // Combined health + performance card (compact)
      h('div', { className: 'card card-pad', style: { marginBottom: 8 } },
        // Top row: health grade + perf deltas side by side
        h('div', { style: { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 } },
          // Health badge
          h('div', { style: { display: 'flex', alignItems: 'center', gap: 6, minWidth: 80 } },
            h('div', { style: { fontSize: 26, fontWeight: 900, color: health.color, lineHeight: 1 } }, health.score),
            h('div', null,
              h('div', { style: { fontSize: 10, fontWeight: 700, color: health.color } }, 'Grade ' + health.grade),
              h('div', { style: { fontSize: 10, color: '#9CA3AF' } }, 'Health')
            )
          ),
          // Vertical divider
          h('div', { style: { width: 1, height: 36, background: '#E5E7EB', flexShrink: 0 } }),
          // Perf numbers — MTD Δ + YTD Δ inline
          h('div', { style: { flex: 1 } },
            p && p[2] != null && h('div', { style: { display: 'flex', gap: 16 } },
              h('div', null,
                h('div', { style: { fontSize: 10, color: '#9CA3AF' } }, 'MTD Δ'),
                h('div', { style: { fontSize: 14, fontWeight: 700, color: pcol(p[2]) } }, fmtPct(p[2])),
                p[0] != null && p[1] != null && h('div', { style: { fontSize: 10, color: '#9CA3AF' } },
                  Math.abs(Math.round(p[1]-p[0])) + ' CE ' + (p[1]>=p[0]?'▲':'▼'))
              ),
              h('div', null,
                h('div', { style: { fontSize: 10, color: '#9CA3AF' } }, 'YTD Δ'),
                h('div', { style: { fontSize: 14, fontWeight: 700, color: pcol(p[5]) } }, fmtPct(p[5])),
                p[3] != null && p[4] != null && h('div', { style: { fontSize: 10, color: '#9CA3AF' } },
                  Math.abs(Math.round(p[4]-p[3])) + ' CE ' + (p[4]>=p[3]?'▲':'▼'))
              )
            ),
            (!p || p[2] == null) && h('div', { style: { fontSize: 11, color: '#9CA3AF' } }, 'No performance data')
          )
        ),
        // Health reasons (only if any)
        healthReasons.length > 0 && h('div', { style: { borderTop: '0.5px solid #F0F0F0', paddingTop: 6 } },
          healthReasons.map((r, i) => h('div', { key: i, style: { fontSize: 11, color: '#6B7280', marginBottom: 2 } }, '· ' + r))
        )
      ),
      ...
      // Notes
      h('div', { className: 'card', id: 'notes-section-' + id, style: { background: '#FFFBF5', borderLeft: '3px solid #D97706' } },
      ...
      // By Segment (collapsible, hidden by default to save space)
      data && data.sg && Object.keys(data.sg).length > 0 && h('div', null, (function() {
        const [segOpen, setSegOpen] = [false, ()=>{}]; // non-interactive summary only
        return null; // removed inline — surfaced in pre-call helper instead
      })()),

      // Removed: standalone performance grid (now folded into combined card above)
      // Legacy reference kept for data binding:
      // Pre-call helper
      h('div', { className: 'card' },
      ...
      ddItems && ddItems.length > 0 && h(DistroDriveSection, {
        ...
        onMarkAll: () => {
          ddItems.forEach(item => {
            if (getVoidState(persist.vs, id, 'dd', item.sku) === 0) {
              dispatch({ type: 'VS', id, voidType: 'dd', sku: item.sku, value: 1 });
            }
          });
        },
        tc,
      ...
      // Weekly objective — clears each new week
      h('div', { className: 'card card-pad', style: { marginBottom: 8 } },
        h('div', { className: 't-cap', style: { marginBottom: 6, color: '#6B7280' } }, "This Week's Objective"),
        h('textarea', {
          className: 'notes-textarea',
          style: { minHeight: 52, fontSize: 13 },
          value: weeklyObj,
          placeholder: "What's the goal for this account this week?",
          onChange: ev => dispatch({ type: 'OBJ', key: id, value: ev.target.value })
        })
      ),

      // Constellation pricing reminder — off-prem accounts only
      data && data.t !== 'O' && h('div', {
        className: 'card card-pad',
        style: { marginBottom: 8, background: '#EFF6FF', border: '1px solid #BFDBFE' }
      },
        h('div', { style: { fontSize: 12, fontWeight: 700, color: '#1D4ED8', marginBottom: 3 } }, '💲 Constellation Pricing — Discuss Today'),
        h('div', { style: { fontSize: 11, color: '#3730A3', lineHeight: 1.5 } },
          'Modelo Cans vs Bottles · Modelo Oro · Corona Premier · Pricing tiers'
        )
      ),
...
// ── Distro Drive Overview Screen ─────────────────────────────────────────────
// Shows territory-wide placement progress per SKU, grouped by brand family.
function DistroDriveOverview({ state, onClose }) {

  // Count sold-in placements per SKU across ALL accounts in persist.vs
  function countSoldIn(sku) {
    let count = 0;
    Object.values(state.persist.vs).forEach(acct => {
      if (acct.dd && acct.dd[sku] === 1) count++;
    });
    return count;
  }

  // Gather every unique SKU across all dd items in RDATA
  const allSkus = {};
  Object.values(RDATA).forEach(dayAccts => {
    dayAccts.forEach(acct => {
      (acct.dd || []).forEach(item => {
        if (!allSkus[item.sku]) allSkus[item.sku] = 0;
      });
    });
  });

  // Build rows with sold-in count and target
  const rows = Object.keys(allSkus).map(sku => {
    const target  = DD_TARGETS[sku] || 1;
    const soldIn  = countSoldIn(sku);
    const pct     = Math.min(100, Math.round((soldIn / target) * 100));
    const done    = soldIn >= target;
    return { sku, target, soldIn, pct, done, family: ddBrandFamily(sku) };
  });
  ...
  // Within each family: incomplete first (sorted by pct desc), then complete
  Object.keys(grouped).forEach(fam => {
    grouped[fam].sort((a, b) => {
      if (a.done !== b.done) return a.done ? 1 : -1;
      return b.pct - a.pct; // closest to done first
    });
  });

  const families = DD_BRAND_ORDER.filter(f => grouped[f]);
  const totalSkus  = rows.length;
  const totalDone  = rows.filter(r => r.done).length;
  const totalPct   = totalSkus > 0 ? Math.round((totalDone / totalSkus) * 100) : 0;

  return h('div', { className: 'page' },
    // Header
    h('div', { style: { background: '#2A2825', paddingTop: 'max(12px, env(safe-area-inset-top))' } },
      h('div', { style: { padding: '0 14px 10px', display: 'flex', alignItems: 'center', gap: 10 } },
        h('button', { className: 'btn btn--ghost-inv btn--sm', onClick: onClose }, '← Back'),
        h('div', { style: { flex: 1 } },
          h('div', { className: 't-title t-white' }, 'Distro Drive — Territory'),
          h('div', { style: { color: 'rgba(255,255,255,0.5)', fontSize: 11, marginTop: 2 } },
            totalDone + ' of ' + totalSkus + ' SKUs at target  (' + totalPct + '%)'
          )
        )
      ),
      // Territory-wide progress bar
      h('div', { style: { height: 6, background: 'rgba(255,255,255,0.15)', margin: '0 0 0 0' } },
        h('div', { style: {
          height: 6,
          width: totalPct + '%',
          background: totalPct === 100 ? '#22C55E' : '#F59E0B',
    ...
    h('div', { style: { padding: 12 } },
      families.map(fam =>
        h('div', { key: fam, style: { marginBottom: 16 } },
          // Family header
          h('div', { style: {
          ...
          // SKU rows
          h('div', { className: 'card' },
            grouped[fam].map((row, i) =>
              h('div', { key: row.sku, style: {
                padding: '10px 12px',
                borderBottom: i < grouped[fam].length - 1 ? '0.5px solid #F0F0F0' : 'none',
                opacity: row.done ? 0.55 : 1,
              }},
                // SKU name + count
                h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 } },
                  h('div', { style: { fontSize: 13, color: '#1F2937', flex: 1, marginRight: 8, lineHeight: 1.3 } }, row.sku),
                  h('div', { style: { display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 } },
                    row.done
                      ? h('span', { style: {
                          fontSize: 11, fontWeight: 700, color: '#166534',
                          background: '#DCFCE7', borderRadius: 10, padding: '2px 8px'
                        }}, '✓ Done')
                      : h('span', { style: { fontSize: 12, fontWeight: 700, color: '#B45309' } },
                          row.soldIn + ' / ' + row.target)
                  )
                ),
                // Progress bar
                h('div', { style: { height: 5, background: '#F3F4F6', borderRadius: 3 } },
                  h('div', { style: {
                    height: 5,
                    width: row.pct + '%',
                    background: row.done ? '#22C55E' : row.pct >= 50 ? '#F59E0B' : '#B45309',
                    borderRadius: 3,
                    transition: 'width 0.3s',
                    minWidth: row.soldIn > 0 ? 6 : 0,
                  }})
                )
              )
            )
...
function App() {
  ...
  const [screen, setScreen]  = useState('home'); // home | summary | manage | settings | distro
  ...
  // Cancel reset confirmation when navigating away from home
  useEffect(() => { setConfirmReset(false); }, [screen, acct]);
  ...
  // ── Sub-screens ─────────────────────────────────────────────────────────────
  if (screen === 'distro') return h('div', null,
    h(DistroDriveOverview, { state, onClose: () => setScreen('home') }),
    toastEl
  );
  ...
  if (acct) return h('div', null,
    h(AccountCard, {
      ...
      weeklyObj:  (state.weekly.obj || {})[acct.acct.id] || '',
  ...
  return h('div', { className: 'page' },
      ...
      h('div', { className: 'app-header__title row-spread' },
        ...
        h('div', { className: 'row' },
          ...
          confirmReset
            ? h('button', {
                className: 'btn btn--ghost-inv btn--sm',
                style: { fontSize: 11, marginLeft: 6, background: 'rgba(185,28,28,0.30)', borderColor: '#FCA5A5', color: '#fff', fontWeight: 700 },
                onClick: () => { dispatch({ type: 'RST' }); setConfirmReset(false); }
              }, '⚠ Confirm?')
            : h('button', {
                className: 'btn btn--ghost-inv btn--sm',
                style: { fontSize: 11, marginLeft: 6, color: '#FCA5A5', borderColor: 'rgba(252,165,165,0.5)' },
    ...
    h('div', { className: 'pad stack' },
      ...
      h('div', { className: 'card card-pad' },
        h('div', { className: 'row-spread', style: { marginBottom: 12 } },
          ...
          h('div', { style: { display: 'flex', gap: 6 } },
            h('button', {
              className: 'btn btn--sm',
              style: { background: 'rgba(180,83,9,0.15)', color: '#B45309', border: '1.5px solid #B45309' },
              onClick: () => setScreen('distro')
            }, '⭐ Distro'),
            h('button', {
              className: 'btn btn--sm',
      ...
      toastEl,
      (function() {
        // Day-level distro progress bar
        const ddTotal  = dayAccts.reduce((sum, r) => {
          const d = getAcctData(r.id); return sum + (d && d.dd ? d.dd.length : 0);
        }, 0);
        const ddSoldIn = ddTotal > 0 ? dayAccts.reduce((sum, r) => {
          const d = getAcctData(r.id);
          if (!d || !d.dd) return sum;
          const acctVs = (state.persist.vs[r.id] || {}).dd || {};
          return sum + d.dd.filter(item => (acctVs[item.sku] || 0) === 1).length;
        }, 0) : 0;
        const ddPct = ddTotal > 0 ? Math.round((ddSoldIn / ddTotal) * 100) : 0;
        if (ddTotal > 0) return h('div', { className: 'card', style: { marginBottom: 8 } },
          h('div', { style: { padding: '8px 14px 4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
            h('span', { style: { fontSize: 12, fontWeight: 700, color: '#B45309' } }, '⭐ Distro Drive — ' + dc.dayName),
            h('span', { style: { fontSize: 12, color: '#6B7280' } }, ddSoldIn + ' / ' + ddTotal + ' SKUs  (' + ddPct + '%)')
          ),
          h('div', { style: { height: 6, background: '#FED7AA', margin: '0 14px 10px', borderRadius: 3 } },
            h('div', { style: { height: 6, width: ddPct + '%', background: ddPct === 100 ? '#166534' : '#B45309', borderRadius: 3, transition: 'width 0.3s' } })
          )
        );
        return null;
      })(),
      h('div', { className: 'card' },
        dayAccts.length === 0
          ...
          : dayAccts.map((r, i) => {
            ...
            // Quick win: 1-2 SKUs from clearing a section entirely
            const ddOpenCount  = dd.length - dd.filter(item => (ddVs[item.sku] || 0) > 0).length;
            const cvOpenCount  = cv.length - cv.filter(v => (cvVs[v.s] || 0) > 0).length;
            const suOpenCount  = (data ? data.su : []).length - (data ? data.su : []).filter(skuIdx => (suVs[SKUS[skuIdx]] || 0) > 0).length;
            const isQuickWin   = (ddOpenCount > 0 && ddOpenCount <= 2) ||
                                  (cvOpenCount > 0 && cvOpenCount <= 2) ||
                                  (suOpenCount > 0 && suOpenCount <= 2);
            ...
            return h('div', {
              ...
              // Star = no distro items needed at this stop
              (!data || !data.dd || data.dd.length === 0) && h('span', {
                style: { fontSize: 12, color: '#166534', marginLeft: 4, fontWeight: 700 }
              }, '✓'),
              isQuickWin && h('span', { style: { fontSize: 11, color: '#166534', marginLeft: 4, fontWeight: 700 } }, '⚡'),
