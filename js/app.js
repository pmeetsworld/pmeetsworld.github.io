...
function VoidSection({ title, subtitle, items, states, onTap, accentColor, stateDef, isObj }) {
  ...
  return h('div', { className: 'card', style: { marginBottom: 8 } },
    ...
    open && h('div', { className: 'collapse-body' },
      activeVis.length < items.length && h('div', { className: 'void-toggle-bar' },
        h('button', {
          className: 'btn btn--sm btn--pill',
...
function DistroDriveSection({ items, allPersistVs, vsGet, onTap, onMarkAll, tc }) {
  ...
  // Group by brand family, sort sold-in to bottom within each group
  const grouped = {};
  items.forEach(item => {
    const fam = ddBrandFamily(item.sku);
    if (!grouped[fam]) grouped[fam] = [];
    grouped[fam].push(item);
  });
  // Sort each group: by type first (Y→B→R), then sold-in last
  const typeOrder = { Y: 0, B: 1, R: 2 };
  Object.values(grouped).forEach(group => {
    group.sort((a, b) => {
      // Sold-in always last
      const aSold = vsGet(a.sku) === 1 ? 1 : 0;
      const bSold = vsGet(b.sku) === 1 ? 1 : 0;
      if (aSold !== bSold) return aSold - bSold;
      // Within same sold state: sort by type
      return (typeOrder[a.type] || 0) - (typeOrder[b.type] || 0);
    });
  });
  // Order families by priority
  const famOrder = DD_BRAND_ORDER.filter(f => grouped[f]);
  ...
  return h('div', { className: 'card', style: { marginBottom: 8, border: '2px solid #B45309' } },
    ...
    open && h('div', null,
      ...
      famOrder.map(fam =>
        h('div', { key: fam },
          h('div', { style: { padding: '5px 12px 3px', background: '#FAFAFA', borderBottom: '0.5px solid #F0F0F0', borderTop: '0.5px solid #F0F0F0' } },
            h('span', { style: { fontSize: 11, fontWeight: 700, color: '#6B7280', textTransform: 'uppercase', letterSpacing: '0.4px' } }, fam)
          ),
          grouped[fam].map((item, i) => {
...
function AccountCard({ acct, data, weeklyDone, weeklyObj, persist, day, dispatch, onBack }) {
  ...
  return h('div', { className: 'page' },
    ...
    h('div', { className: 'pad stack', style: { paddingTop: 12 } },
      ...
      // Account health score summary
      h('div', { className: 'card card-pad', style: { marginBottom: 8 } },
        h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: healthReasons.length ? 6 : 0 } },
          h('div', { style: { fontSize: 28, fontWeight: 900, color: health.color, lineHeight: 1 } }, health.score),
          h('div', null,
            h('div', { style: { fontSize: 11, fontWeight: 700, color: health.color } }, 'Account Health — Grade ' + health.grade),
            h('div', { style: { fontSize: 11, color: '#9CA3AF' } }, 'Performance · Compliance · Voids · Distro')
          )
        ),
        healthReasons.length > 0 && h('div', { style: { borderTop: '0.5px solid #F0F0F0', paddingTop: 6 } },
          healthReasons.map((r, i) => h('div', { key: i, style: { fontSize: 12, color: '#6B7280', marginBottom: 2 } }, '· ' + r))
        )
      ),
