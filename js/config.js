...
// ── Distro Drive placement targets (from Placements Needed by Rep row) ─────────
// Key = SKU name, Value = number of placements needed across the territory.
// When sold-in count across all accounts >= target, SKU is done everywhere.
const DD_TARGETS = {
  "CORONA EX 12PK BT": 6,
  "CORONA EX 18PK BT": 7,
  "CORONA EX 24PK BT": 7,
  "CORONA EX 6PK BT": 6,
  "CORONITA 7OZ 24PK BT": 1,
  "MODELO ESP 12PK BT": 6,
  "MODELO ESP 12PK CAN": 4,
  "MODELO ESP 18PK CAN": 4,
  "MODELO ESP 24OZ CAN": 5,
  "MODELO ESP 24PK BT": 7,
  "MODELO ESP 24PK CAN": 4,
  "MODELO ESP 6PK BT": 5,
  "CORONA NONALC 12PK BTL": 4,
  "CORONA NONALC 6PK BTL": 2,
  "CORONA SUNBREW 12OZ 12PK BTL": 3,
  "CORONA SUNBREW 12OZ 12PK CAN": 3,
  "CORONA SUNBREW 12OZ 6PK BTL": 6,
  "CORONA SUNBREW 24OZ CAN": 4,
  "MODELO CH NA LMYS 12OZ 6PK CAN": 2,
  "MODELO CH SUPR MNGO 24OZ CAN": 2,
  "MODELO NEGRA 12PK CAN": 2,
  "CORONA EX 12PK CAN": 2,
  "CORONA EX 16OZ 4PK CAN": 3,
  "CORONA FAMILIAR 12PK BT": 2,
  "CORONA PREMIER 12PK CAN": 4,
  "CORONA PREMIER 24PK CAN": 1,
  "MODELO CH LSAL 12OZ/12PK CAN": 3,
  "MODELO CH LSAL 16OZ/4PK CAN": 4,
  "MODELO CH LSAL 24OZ CAN": 8,
  "MODELO ESP 16OZ 4PK CAN": 2,
  "MODELO ORO 12 OZ 12 PK CAN": 7,
  "MODELO ORO 12 OZ 24 PK CAN": 1,
  "PACIFICO 12PK CAN": 3,
  "PACIFICO 16OZ/24 4PK DEP CAN": 3,
  "VICTORIA 12PK CAN": 4,
  "CORONA EXTRA PACKAGE": 11,
  "MODELO ESPECIAL DRAFT": 2,
  "MODELO ESPECIAL PACKAGE": 14,
  "CORONA NON-ALCOHOLIC PACKAGE": 3,
  "CORONA SUNBREW PACKAGE": 5,
  "CORONA PREMIER DRAFT": 1,
  "CORONA PREMIER PACKAGE": 8,
  "MODELO NEGRA DRAFT": 2,
  "MODELO ORO PACKAGE": 1,
  "PACIFICO DRAFT": 2,
  "PACIFICO PACKAGE": 6
};
...
function reconcileRoster(savedRoster) {
  ...
  for (const r of updatedRoster) {
    ...
    if (!live) {
      // Skip archiving for EOW/monthly accounts — they're intentionally off-route some weeks
      if (r.freq === 'EOW' || r.freq === 'M') {
        r.status = 'active'; // keep active regardless
      } else if (r.status !== 'archived') {
        // Account not in current RDATA and not EOW/OAM — archive it
        r.status = 'archived';
...
// Returns { score: 0-100, grade: 'A'|'B'|'C'|'D', color, reasons: [] }
// Weights: performance trend 35%, compliance freshness 35%,
//          open voids + distro completion 20%, tasks 10%
function accountHealthScore(id, data, persist) {
  let score = 100;
  const reasons = [];

  // ── Performance trend (35 pts) ─────────────────────────────────────────────
  const p = data && data.p;
  if (p && p[2] != null) {
    const mtd = p[2]; // MTD % change vs prior year
    if      (mtd < -0.30) { score -= 35; reasons.push('MTD down ' + Math.abs(Math.round(mtd*100)) + '% — major volume loss'); }
    else if (mtd < -0.15) { score -= 22; reasons.push('MTD down ' + Math.abs(Math.round(mtd*100)) + '% — below trend'); }
    else if (mtd < -0.05) { score -= 12; reasons.push('MTD slightly down ' + Math.abs(Math.round(mtd*100)) + '%'); }
    else if (mtd >  0.10) { score += 5;  reasons.push('MTD up ' + Math.round(mtd*100) + '% — strong momentum'); }
  } else {
    score -= 10; // no data — can't assess
  }

  // ── Compliance freshness (35 pts) ──────────────────────────────────────────
  const ds = persist.ds || {};
  const fields = ['tags','ooc','rebates','pocm'];
  let compDeduct = 0;
  fields.forEach(field => {
    const val = ds[id + '_' + field];
    const days = daysSince(val || null);
    if      (days === null)  compDeduct += 9; // never logged
    else if (days > 45)      compDeduct += 7; // stale
    else if (days > 21)      compDeduct += 3; // getting old
    // recent = no deduction
  });
  score -= Math.min(35, compDeduct);
  if (compDeduct >= 28) reasons.push('Compliance overdue — tags, OOC, or rebates need attention');
  else if (compDeduct >= 16) reasons.push('Some compliance checks getting stale');

  // ── Open voids + distro completion (20 pts) ────────────────────────────────
  if (data) {
    const acctVs    = persist.vs[id] || {};
    const cvAll     = [...(data.ca || []), ...(data.cn || [])];
    const cvOpen    = cvAll.filter(v => (acctVs.cv && acctVs.cv[v.s]) ? false : true).length;
    const suOpen    = (data.su || []).filter(idx => !(acctVs.su && acctVs.su[SKUS[idx]] && acctVs.su[SKUS[idx]] > 0)).length;
    const plOpen    = (data.pl || []).filter(idx => !(acctVs.pl && acctVs.pl[SKUS[idx]] && acctVs.pl[SKUS[idx]] > 0)).length;
    const ddItems   = data.dd || [];
    const ddOpen    = ddItems.filter(item => !(acctVs.dd && acctVs.dd[item.sku] === 1)).length;
    const totalOpen = cvOpen + suOpen + plOpen;
    const ddDeduct  = ddOpen > 0 ? Math.min(10, Math.floor(ddOpen / 3)) : 0;
    const voidDeduct = totalOpen > 10 ? 10 : totalOpen > 5 ? 7 : totalOpen > 2 ? 4 : totalOpen > 0 ? 2 : 0;
    score -= (ddDeduct + voidDeduct);
    if (ddOpen > 5)    reasons.push(ddOpen + ' distro SKUs still open');
    if (totalOpen > 5) reasons.push(totalOpen + ' open voids/scale-up items');
  }
  ...
  let grade, color;
  if      (score >= 80) { grade = 'A'; color = '#166534'; }
  else if (score >= 65) { grade = 'B'; color = '#B45309'; }
  else if (score >= 45) { grade = 'C'; color = '#B91C1C'; }
  else                  { grade = 'D'; color = '#7F1D1D'; }
